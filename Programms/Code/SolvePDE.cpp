        ...
        for(int j = 0; j <= num_time_steps; ++j) {
            ...
            for(int s = 0; s<3; ++s) {
                ...
                if (!test.G_left_type) {
                    ...
                    double a0 = a(test.K_ptr, x_0 + h, x_0, state_ipp[1], state_ipp[0]);
                    double w0 = w(a0, state_ipp[1], state_ipp[0], h);
                ...
                if (!test.G_right_type) {
                    ...
                    double am = a(test.K_ptr, X, X - h, state_ipp[num_space_steps], state_ipp[num_space_steps-1]);
                    double wn = w(am, state_ipp[num_space_steps], state_ipp[num_space_steps - 1], h);
                    double denom = c * rho * h / (2 * tau) + sigma * am / h;
                    double kappa = sigma * am / h / denom;
                ...
                for (int i = 1; i < num_space_steps; ++i) {
                    x_i += h;
                    double a_i = a(test.K_ptr, x_i, x_i - h, state_ipp[i], state_ipp[i-1]);
                    double a_ip = a(test.K_ptr, x_i + h, x_i, state_ipp[i + 1], state_ipp[i]);
                    As[i] = sigma / h * a_i;
                    Bs[i] = sigma / h * a_ip;
                    Cs[i] = (As[i] + Bs[i] + c * rho * h / tau);
                    Fs[i] = (c * rho * h / tau * state_i[i] +
...
bool infoIterationScheme(double tau, double h, double sigma, PDE_data test, std::string filename="ImpScheme", double EPS=1e-17){
    ...
    if (fpoints.is_open()) {
        ...
        // Открываем файл для записи числа внутренних итераций на каждом шаге
        std::string infoPath = "./OutputData/iterationsAmount_"+filename;
        std::ofstream fiterations(infoPath);
        std::cout << "log[INFO]: Opening a file \"" << infoPath << "\" to write..." << std::endl;

        if (fiterations.is_open()) {
            ...
            for (int j = 0; j <= num_time_steps; ++j) {
                ...
                fiterations << iter_counter << std::endl;
                state_i = state_ipp;
                // Запись в файл состояния
                writeVectorToFile(fpoints, t_i, state_i);
            }
            fiterations.close();
            fpoints.close();
            ...
            std::cout << "log[ERROR]: Couldn't open or create a file for iterations" << std::endl;
        }
    }
    else {
        std::cout << "log[ERROR]: Couldn't open or create a file" << std::endl;
